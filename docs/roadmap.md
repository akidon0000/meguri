...
外部 LLM の方が同定精度が高い可能性がある。[市場調査](market-research.md)で見た通り、
Apple Intelligence 対応端末しか Meguri を使えないという配布面の制約でもあるため、
解説品質の向上だけでなく「対応端末を増やす」目的でも優先度が上がる項目。
...
**何を**: `Entry` に保存済みの緯度経度（[CoreLocationService.swift](../Meguri/Services/CoreLocationService.swift)）を使い、
記録した場所を地図上にピン表示する、あるいは訪問済みの都道府県・国を色分けする画面を追加する。

**なぜ**: [市場調査](market-research.md)で見た「たびのしるし」「経県値」等の日本の旅行記録アプリは
地図×行政区画の可視化が中心的な体験になっている。Meguri は既に位置情報を保存しているため、
可視化を足すだけで同じ体験を提供でき、「旅タブ／場所タブ」と並ぶ振り返り導線になりうる。

**論点（着手時に詰める）**:

| 論点 | メモ |
|---|---|
| 粒度 | ピンを打つだけか、都道府県・国単位で塗るかで実装規模が変わる。後者は行政区画データの逆引きが要る |
| 「旅タブ／場所タブ」との関係 | 別画面にするか、いずれかのタブの中に地図ビューとして統合するか |
| 位置情報が無いエントリ | 手動で撮った写真を後から選ぶ運用の場合、位置情報が無いケースをどう地図に出すか（出さない、が妥当そう） |
